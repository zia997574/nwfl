...
function updateCart() {
  ...
  cartProducts.forEach(function(product, index) {
      ...
      let price = getNumericPrice(product.price);
      if (!isNaN(price)) {
  ...
  // Attach event listeners for quantity buttons and trash icons
  document.querySelectorAll(".quantity-decrement").forEach(button => {
      button.addEventListener("click", function () {
          decrementQuantity(this.dataset.index);
      });
  });

  document.querySelectorAll(".quantity-increment").forEach(button => {
      button.addEventListener("click", function () {
          incrementQuantity(this.dataset.index);
      });
  });
...
// Function to properly format price values
function formatPrice(price) {
  let numericPrice = getNumericPrice(price);
  return numericPrice.toFixed(2);
}

// Function to extract the correct numeric price
function getNumericPrice(price) {
  let cleanedPrice = price.replace(/[^\d.]/g, ''); // Remove non-numeric characters
  let numericPrice = parseFloat(cleanedPrice);

  // If the price is mistakenly small (e.g., 0.35 instead of 350), fix it
  if (numericPrice < 10 && cleanedPrice.includes(".")) {
      numericPrice *= 100; // Convert from decimal representation
  }
